...
class ClustersBuilder {
  ...
  ClustersBuilder(Projection projection, Options options,
      ArrayList<ClusterPoint> initialClusteredPoints) {
  ...
  private LatLngBounds getExpandedBounds(LatLngBounds bounds) {
    if (bounds != null && options != null) {
      ...
      if (spans180Meridian == false) {
        ...
        distanceFromEastToWest =
            (180 + bounds.northeast.longitude) + (180 - bounds.southwest.longitude);
      }
      ...
      LatLng newNortheast = new LatLng(bounds.northeast.latitude + expandLatitude,
          bounds.northeast.longitude + expandLongitude);
      LatLng newSouthwest = new LatLng(bounds.southwest.latitude - expandLatitude,
          bounds.southwest.longitude - expandLongitude);
  ...
  private void addIfNecessary(InputPoint point, Projection projection, LatLngBounds bounds) {
    if (bounds != null
        && bounds.contains(point.getMapPosition())
        && !releventInputPointsSet.contains(point)) {
