...
public class SampleActivity extends FragmentActivity
    implements GenerateCallback, ProcessingListener {
    ...
    if (map != null && options != null && inputPoints != null) {
      try {
        if (restoreCameraPosition != null) {
           ...
           */
          float zoom =
              options.geographicDistribution == GeographicDistribution.NearTwoToasters ? 11 : 4;
          map.moveCamera(
              CameraUpdateFactory.newLatLngZoom(MarkerData.TwoToasters.getLatLng(), zoom));
        }
        initClusterkraf();
  ...
  private void applyDemoOptionsToClusterkrafOptions(com.twotoasters.clusterkraf.Options options) {
    ...
    try {
      interpolator =
          (Interpolator) Class.forName(this.options.transitionInterpolator).newInstance();
    ...
    options.setZoomToBoundsPadding(
        getResources().getDrawable(R.drawable.ic_map_pin_cluster).getIntrinsicHeight());
